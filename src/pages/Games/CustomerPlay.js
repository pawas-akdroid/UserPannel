...
const OTPComponent = ({ setOtpVerify, setPhone, phone }) => {
    ...
    const handleUserPhone = () => {
        if (confirm("Are you sure?")) {
            TokenUrl().get(`/get-transfer-token?customer=${phone}`).then((res) => {
                setOtpVerified(true)
                console.log(res)
    ...
    const handleOtpVerify = () => {
        TokenUrl().post(`/verify-transfer-token?customer=${phone}`, { "token": OTP }).then(res => {
            console.log(res)
            setOtpVerify(true)
        }).catch((err) => {
            ErrorHandler(err)
...
const GameComponent = ({ id, phone }) => {
    ...
    useEffect(() => {
        if (OTP.length === 6) {
            ...
            TokenUrl().post('/game', { game_id: id, user_id: phone, "chosen_number": selectedNumbers }).then(res => {
