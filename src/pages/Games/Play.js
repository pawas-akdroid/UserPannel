...
const [loading, setLoading] = useState(true)
  ...
  useEffect(() => {
    ...
    TokenUrl().get(`game/${uid}`).then(res => {
      ...
      setAllowed(res?.data?.data?.allowed_numbers)
      setPlusOne(res?.data?.data?.extra)
      addInCircle(res?.data?.data?.total_numbers)
    }).catch((err) => {
      ErrorHandler(err)
    })
  }, [])
  ...
  useEffect(() => {
    if (OTP.length === 6) {
      ...
      TokenUrl().post('/game', { game_id: uid, "chosen_number": selectedNumbers }).then(res => {
