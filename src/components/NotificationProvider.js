...
export const ErrorHandler = (err) => {
  ...
  else if (err?.response?.status === 406) return ErrorNotification({ title: "Validation Error!", message: err?.response?.data?.error?.errors[0].msg })
  else if (err?.response?.status === 403) return ErrorNotification({ title: "Forbidden Error!", message: err?.response?.data?.error?.errors[0].msg })
